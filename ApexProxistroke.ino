...
#define VERSION_DATE "v1.5.3 - 15/03/2026"
...
int menuLevel = 0; 
unsigned long lastMove = 0; // Global timer
...
void wipeSystem() {
    ...
    for(int i=0; i<APP_COUNT; i++) {
        strcpy(apps[i], "Empty Slot"); strcpy(usernames[i], "user"); strcpy(passwords[i], "pass");
    }
    saveDataToEEPROM();
}
...
void serialEdit() {
    ...
    if (slot >= 0 && slot < APP_COUNT) {
        ...
        Serial.println(F("Done. Type WRITE to save."));
    }
}
...
void setup() {
    ...
    // Check for "Cheat Code" reset on boot
    ...
    if (digitalRead(BUTTON_SELECT) == LOW && (millis() - lastMove > 500)) {
        ...
        if (menuLevel == 2) { 
            ...
            if (!masterKeySet || isAuthorised()) { 
                ...
                Serial.println(F("\nCONFIG MODE: DUMP, EDIT, AUTH 0, WRITE, EXIT"));
            } else { updateMenu(); }
        } 
        else {
            if (isAuthorised()) {
                lcd.clear(); lcd.print(F("TYPING..."));
                typeUK(usernames[currentAppIndex]); Keyboard.write(KEY_TAB);
                typeUK(passwords[currentAppIndex]); Keyboard.write(KEY_RETURN);
                delay(2000); 
            }
            updateMenu();
        }
